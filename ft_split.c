...
/*   Updated: 2023/03/21 13:32:44 by cyazid-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int  count_words(char *str, char sep)
{
    ...
    while (str[a] != '\0')
    {
        if (char_is_sept(str[a + 1], sep) == 1 && char_is_sept(str[a], sep) == 0)
            words++;
        a++;
    }
    return (words);
    ...
    while (str[a] != '\0')
        ...
        else
        {
            ...
            while (char_is_sept (str[a + b], sep) == 0)
                b++;
            if ((split[word] = (char*)malloc(sizeof(char) * (b + 1))) == NULL)
            {
                while(word > 0)
                    free (split[--word]);
                return (NULL);
...

{
    ...
    if ((rs = (char **)malloc(sizeof(char*) * (word + 1))) == NULL)
        return(NULL);
    rs[word] = 0;
